...
class RadixTree:
    ...
    def get_corrections(self, word):
        """
            ...
            алгоритма Дамерау-Левенштейна (в худшем случае) будет равна О(
            ^2) где n - длина проверяемого слова
    ...
    def __is_dam_lev_distance_one(s1, s2):
        """
            ...
            завершения будет увеличиваться. По этому в среднем данная достаточно хорошо оптимизирована. Если учитывать
            не только 26 букв английского алфавита, а например 100 разных символов, то сложность будет
            достаточно близка к константной тк 98% случаев будут завершаться за 2 итерации.
            (Хоть в реальности буквы в словах распределены не равномерно, но для грубой оценки этого достаточно)
